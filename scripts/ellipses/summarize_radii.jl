...
plot_cdf!(p_tcdf, (bounds_ransag.t)[filter_combined .&& bounds_ransag.t .> 0]; label="RANSAG", c="#2ca02c")
plot_cdf!(p_tcdf, slem_t[1,:]; label="t_1",c=color_ours,lw=2)
plot_cdf!(p_tcdf, slem_t[2,:]; label="t_2",c=color_ours,lw=2)
plot_cdf!(p_tcdf, slem_t[3,:]; label="t_3",c=color_ours,lw=2)
Plots.plot!(xscale=:log10)
Plots.plot!(yticks=0:0.2:1)
Plots.plot!(p_tcdf,xlim=[0.005,10])

# angular CDF (radius)
if sdporder == 2
    p_acdf = Plots.plot(ylabel="CDF", title="Angular Bounds (Order $sdporder)")
end
plot_cdf!(p_acdf, (bounds_ransag.θ)[filter_combined .&& bounds_ransag.θ .> 0]; label="RANSAG", c="#2ca02c")
