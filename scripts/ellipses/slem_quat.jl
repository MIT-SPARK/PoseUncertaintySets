...
savepath = "../data/$dataset/ellipse_slem_quat_o$(order)_$(round(Int,α*100))_$(string(p)).dat"
...
println("\nStarting S-Lemma (quaternion)")
data = DataFrame()
ellipses = Dict()
for object_id in object_ids
    println("\n------------$object_id------------")
    # solve!
    Hs, statuses, times, gaps = dataset_slem(keypoint_data, poses, object_id; order=order, quat=true)
