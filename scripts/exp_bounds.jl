...
α = 0.1
p = Inf
save_path = "../data/bounds$(string(p))_alpha$(round(Int,α*100)).dat"
...
## OURS QUAT
println("\nStarting S-Lemma + Refinement..")
bounds_slem_quat = DataFrame()
data_slem_quat = Dict()
for object_id in object_ids
    println("\n------------$object_id------------")
    # solve!
    Hs, Δθs, Δts, statuses, times, gaps = dataset_slem_bounds_quat(keypoint_data, solns_g2, object_id)
    ...
    # save
    bounds_obj = DataFrame(frame=collect(keys(Hs)), id=object_id, time_s=times[1,:],
                θ=Δθs, time_r=times[2,:],
                tu1=trans[1,1:3:end], tu2=trans[1,2:3:end], tu3=trans[1,3:3:end],
                tl1=trans[2,1:3:end], tl2=trans[2,2:3:end], tl3=trans[2,3:3:end], time_t=times[3,:],
                optimal=[MOI.OPTIMAL in statuses[frame][2:end] || MOI.ALMOST_OPTIMAL in statuses[frame][2:end] for frame in keys(statuses)])
    global bounds_slem_quat, data_slem_quat
    bounds_slem_quat = [bounds_slem_quat; bounds_obj]
    data_slem_quat[object_id] = (statuses, gaps)
end
println("")
