...
"""
    sdp_pose, status, feas, _ = central_pose_l2(q_front, q_backproj, q_eqs;[])
...
## Optional Arguments
- `lowerb=nothing`: lower bound the margins by 0 (irrelevant if `analytic=true`)
- `upperb=0.8`: controls constraint `margin < upperb*r` ∈ (0,1]
- `silent=false`: should we print things?
"""
function central_pose_l2(y, r, b, camK; lowerb=nothing, upperb=0.8, silent=false)
    # @assert (upperb <= 1. && upperb > 0.) "upperb ∉ [0,1]"
    ...
    return (R_est, t_est), data.SDP_status, vars_proj, gap
end