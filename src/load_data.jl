...
function load_keypoint_data_ycbv(cal_fn, frames_cal, path_kpts3d, parent_test, detections_test, parent_cal, detections_cal; p=2, α=0.1)
    ...
    kpts_test = load_raw_keypoints(detections_test)

    # load cal data
    gt_cal = Dict()
    for folder in readdir(parent_cal)
        gt_f = load_gt(parent_cal*"/$folder")
        foldernum = parse(Int, folder)
        gt_f = Dict(foldernum*10000 + k => v for (k,v) in pairs(gt_f))
        merge!(gt_cal, gt_f)
    end
    kpts_cal = load_raw_keypoints(detections_cal)
...
function load_keypoint_data(cal_fn, dataset="lmo"; p=2, α=0.1)
    ...
    elseif dataset == "ycbv"
        ...
        parent_cal = "../data/$dataset/cal"
        frames_cal = []
        return load_keypoint_data_ycbv(cal_fn, frames_cal, path_kpts3d, parent_test, detections_test_path, parent_cal, detections_cal_path; p=p, α=α)
    elseif dataset == "cast"
